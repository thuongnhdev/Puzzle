...
namespace EventDispatcher
{
	...
	public class EventDispatcher : MonoBehaviour
    {
		...
		void Awake ()
		{
			...
			if (s_instance != null && s_instance.GetInstanceID() != this.GetInstanceID())
			{
				...
				DataCore.Debug.LogWarning(string.Format("An instance of EventDispatcher already exist : <{0}>, So destroy this instance : <{1}>!!", s_instance.name, name));
				Destroy(gameObject);
			}
			else
			{
	...
	public static class EventDispatcherExtension
	{
		...
		{
            if (EventDispatcher.IsDestroyed)
            {
                return;
            }
